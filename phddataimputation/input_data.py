...
for i in range(len(df) - 1 - 1):
    #if (i + 1 + 24 + 24) < len(df)
    if i == 0:
        x1 = np.reshape(df[i:i+1, 1].astype(float), [1, -1])
        # x2 = np.reshape(df[i + 1 + 1 : i + 1 + 1 + 1, 1].astype(float), [1, -1])
    else: 
        x1 = np.vstack((np.reshape(df[i:i+1, 1].astype(float), [1, -1]), x1))
        # x2 = np.vstack((np.reshape(df[i + 1 + 24 : i + 1 + 24 + 24, 1].astype(float), [1, -1]), x2))
    y.extend([float(df[i+1, 1])])
x1 = np.round(x1, decimals=2)
# x2 = np.round(x2, decimals=2)
y = np.round(y, decimals=2)
np.savetxt('data/trainingData/x_1_feature.csv', x1, fmt='%f', delimiter=',')
